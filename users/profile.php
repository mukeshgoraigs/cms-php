...
//echo "hello".$regDate; exit;
$query=mysqli_query($con,"update users set fullName='$fname',contactNo='$contactno',address='$address',State='$state',country='$country',pincode='$pincode',regDate='$regDate' where userEmail='".$_SESSION['login']."'");
if($query)
{
...
<label class="col-sm-2 col-sm-2 control-label">User Email </label>
 <div class="col-sm-4">
<input type="email" name="useremail" required="required" value="<?php echo htmlentities($row['userEmail']);?>" class="form-control" >
</div>
 </div>
...
<div class="col-sm-4">
<input type="text" name="regdate" onfocus="(this.type='date')" placeholder="<?php echo htmlentities($row['regDate']);?>" class="form-control">
 </div>
</div>
